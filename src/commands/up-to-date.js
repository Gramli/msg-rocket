...
export async function handleUpToDate(flags) {
    ...
    const masterBranch = flags.hasFlag(mainBranchFlag) ? flags.getFlagValue(mainBranchFlag) : "master";
    await fetchOrigin();
    ...
    if(isMaster) {
        if(await hasChanges() && (await countBehindOrigin(masterBranch)) > 0) {
            ...
            await pullOriginFFOnly();
            log(LOG_LEVELS.INFO, `Pulled latest changes from origin/${masterBranch}.`);
            await applyStash(stashRef);
            log(LOG_LEVELS.INFO, "Re-applied stashed changes.");
        } else {
            log(LOG_LEVELS.INFO, `Repository is up-to-date with origin/${masterBranch}. No staged changes.`);
        }
    } else {
        ...
        if(await hasChanges() && (await countBehindOrigin(branch)) > 0) {
            ...
            log(LOG_LEVELS.INFO, `Repository is up-to-date with origin/${branch}. No staged changes.`);
        }
